...
export default function TravelTab() {
  ...
  const handleDeleteTrip = async (id: string) => {
    ...
    try {
      ...
      if (activeTripId === id) {
        setActiveTripId(trips.length > 1 ? trips[0].id : null);
      }
    } catch (err) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto px-4 select-none">
      
      {/* 1. Left Column: Trips List */}
      <div className="space-y-4 md:col-span-1">
        {error && (
          <div className="bg-rose-50 border border-rose-100 text-rose-700 p-3 rounded-xl text-center text-xs">
            {error}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="text-center py-10 bg-white rounded-2xl border border-stone-100 p-6 space-y-2">
          ...
          <div className="space-y-2">
            {trips.map((trip) => {
              ...
              return (
                ...
                >
                  <h4 className="font-bold text-xs text-stone-800 font-serif leading-tight">{trip.name}</h4>
                  ...
                  <button
                    ...
                    className="absolute top-4 right-4 p-1 hover:bg-stone-50 rounded text-stone-300 hover:text-red-500 transition-colors"
                  >
      ...
      {/* 2. Right Column: Packing Checklist Details */}
      <div className="md:col-span-2 space-y-4">
        {activeTrip ? (
          <div className="bg-white rounded-3xl border border-stone-150 p-6 space-y-6 shadow-sm">
            
            {/* Header info */}
            <div className="flex justify-between items-start border-b border-stone-50 pb-4">
              <div className="space-y-1">
                <span className="text-[10px] bg-stone-100 text-stone-500 font-bold uppercase px-2 py-0.5 rounded tracking-wide">
                  Chi tiết đóng gói Vali
                </span>
                <h3 className="font-bold text-lg text-[#2A2521] font-serif leading-none pt-1">{activeTrip.name}</h3>
                <p className="text-xs text-stone-400 flex items-center gap-2 font-medium">
                  <span className="flex items-center gap-0.5">
                    <MapPin className="h-3 w-3 text-stone-300" />
              ...
              <button
                ...
                className="flex items-center gap-1 px-4 py-2 bg-[#8A9A5B] hover:bg-[#72804b] text-white text-xs font-bold rounded-xl transition-all shadow-xs"
              >
            ...
            {/* Checklist items */}
            {tripItems.length === 0 ? (
              <div className="text-center py-12 border border-dashed border-stone-200 rounded-2xl p-6 space-y-3 bg-stone-50/20">
              ...
              <div className="divide-y divide-stone-100">
                {tripItems.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center justify-between py-3.5 hover:bg-stone-50/50 rounded-xl px-2 transition-colors group"
                  >
                    <div 
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-stone-150 p-6 space-y-3 shadow-xs h-full">
